...
def createCircuit(input:torch.tensor, bias:torch.tensor, W:ModuleList, dimensions:list, **params):
    """
    ...
    for i in range(n_layers-2): # exclude out, input layers: 0~n-2
      ...
      G = W[i].data
       #print(1/G)
      #Rarray pre1->pre2
      circuit.subcircuit(Rarray(inNodes,outNodes,pre1=Prefix1,pre2=Prefix2,L=params["L"][i], U=params["U"][i], G = G))
    ...
    G = W[-1].data
    circuit.subcircuit(Rarray(inNodes,outNodes, pre1 =lastPrefix, pre2='o',L=params["L"][-1], U=params["U"][-1], G= G))
