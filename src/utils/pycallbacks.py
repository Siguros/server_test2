...
class AdvLoggerCallback(Callback):
    ...
    def on_train_epoch_end(self, trainer: Trainer, pl_module: pl.LightningModule) -> None:
        self.log_everything(
            pl_module.backbone, step=trainer.current_epoch, key_suffix="_epoch"
        )
    ...
    @abstractmethod
    def log_histogram(
        self, key, data, step: int = None, key_suffix: str = "", **kwargs
    ):
        pass
        ...
        **kwargs,
        ...
        if Vdrops is not None:
            for idx, value in enumerate(Vdrops):
                self.log_histogram(
                    f"Vdrop_{key_prefix}_{idx}", value, step, key_suffix, **kwargs
                )

    # TODO: add log metrics
    def log_everything(
        self, backbone: Union[EP, AnalogEP], key_suffix: str, step: int = None, **kwargs
    ):
        if self.log_optn["Vdrops"]:
            self.log_Vdrops(
                backbone.fdV, step, key_prefix="free", key_suffix=key_suffix, **kwargs
            )
            self.log_Vdrops(
        ...
        for key, value in Weights:
            if self.log_optn["histogram"]:
                ...
                self.log_histogram(
                    key + ".grad", value.grad, step, key_suffix, **kwargs
                )
        ...
        **kwargs,
        ...
        for key, value in scalars.items():  # l
            self.logger.add_scalars(
                layer_name + key_suffix, {key: value}, step, **kwargs
            )
...
class WandbLoggerCallback(AdvLoggerCallback):
    ...
    def on_train_start(self, trainer: Trainer, pl_module: pl.LightningModule) -> None:
        ...
        self.logger = (
            WandbLogger() if self.logger is None else self.logger
        )  # for fast_dev_run
