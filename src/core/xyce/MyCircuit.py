...
from PySpice.Spice.Netlist import FixedPinElement
...
class SimpleElement():
    ...
    def copyFromElement(cls, element):
        value = element.dc_value \
            if hasattr(element, 'dc_value') else element.subcircuit_name
...
class MyCircuit:
    ...
    def __init__(self, title, includes,  options = None):
        ...
        self.header = '.title ' + title + os.linesep + \
            self.appendlist('.include ',includes)
    ...
    def _appendnodes(self):
        ...
        """
        Rarrays = [self.raw_elements[key].nodes for key in self.raw_elements.keys() if key.startswith('XR')]
